...
namespace refusion {
    ...
    namespace tsdfvh {
        ...
        void TsdfVolume::Init(const TsdfVolumeOptions &options) {
            ...
            //gpuErrchk(cudaStreamCreateWithPriority(&transform_stream_, cudaStreamNonBlocking, least_prio))
            for (int i = 0; i < options_.num_render_streams; i++) {
        ...
        /*__global__ void transformFusionKernel(
                TsdfVolume* volume,
                float4x4 transform, 
                TsdfVolume* new_volume, 
                float3 lower_corner, 
                float3 upper_corner){
            ...
            if (index == 2046)
                printf("transformFusionKernel %d\n", index);

            float3 size = lower_corner - upper_corner;
            size = make_float3(fabs(size.x), fabs(size.y), fabs(size.z));

            int3 grid_size =
                make_int3(size.x / volume->GetOptions().voxel_size, size.y / volume->GetOptions().voxel_size, size.z / volume->GetOptions().voxel_size);
            
            int grid_linear_size = grid_size.x * grid_size.y * grid_size.z;//8000,000
            ...
            for (int i = index; i < grid_linear_size; i += stride) {
                ...
                float3 world_position = make_float3(
                    static_cast<float>(grid_position.x) * volume->GetOptions().voxel_size + lower_corner.x,
                    static_cast<float>(grid_position.y) * volume->GetOptions().voxel_size + lower_corner.y,
                    static_cast<float>(grid_position.z) * volume->GetOptions().voxel_size + lower_corner.z);

                Voxel voxel = volume->GetVoxel(world_position);
                if (index == 2046)
                    printf("voxel.sdf = %d \n", voxel.sdf);
                
                // transform to new world pos
                float3 world_new = make_float3(
                    transform.m11*world_position.x + transform.m12*world_position.y + transform.m13*world_position.z + transform.m14,
                    transform.m21*world_position.x + transform.m22*world_position.y + transform.m23*world_position.z + transform.m24,
                    transform.m31*world_position.x + transform.m32*world_position.y + transform.m33*world_position.z + transform.m34);
                //if (index == 2046)
                //printf("world_new.x = %d\n", world_new.x);

                float3 world_position_new = transform * world_position;
                if (world_new.x != world_position_new.x || world_new.y != world_position_new.y || world_new.z != world_position_new.z)
                    printf("world_new != world_position_new");

                new_volume->UpdateVoxel(world_new, voxel);

                Voxel new_voxel = new_volume->GetVoxel(world_new);
                if (index == 2046)
                    printf("new_voxel.sdf = %d \n", new_voxel.sdf);
                //volume->UpdateVoxel(world_position_new, voxel);
            }
        }

        void TsdfVolume::transformFusion(
            ...
            //         threads_per_block;
            int thread_blocks =
                    (options_.num_blocks * options_.block_size + threads_per_block - 1) /
                    threads_per_block;

            // wait for render events to complete
            ...
            // Call Kernels
            int mem = 0;
            printf("Launching transformFusionkernel\n");
            clock_t started = clock();
            if(transform_stream_){
                transformFusionKernel<<<thread_blocks, threads_per_block, mem, transform_stream_>>>(
                        this, transform, new_volume, lower_corner, upper_corner);
            }
            ...
            printf("Sync After transformFusionkernel kernel: %f ms\n", MilliSecondsTaken);
...
//            gpuErrchk( cudaDeviceSynchronize() )
        ...
        __global__ void GenerateRgbDepthKernel(TsdfVolume *volume, RgbdSensor sensor,
            ...
            int stride = blockDim.x * gridDim.x;
            int size = sensor.rows * sensor.cols;
            //printf("GenerateRgbDepthKernel %d\n", index);
