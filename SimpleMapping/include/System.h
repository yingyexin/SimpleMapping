...
public:
    ...
    // Initialize the SLAM system. It launches the Local Mapping, Loop Closing, Viewer threads, and MvsBackend. 
    System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor, const bool bUseViewer = true, const int initFr = 0, const string &strSequence = std::string(), const string &strModelFile = std::string());
    ...
    // Save keyframe trajectory wrt. world frame instead of the first keyframe
    void SaveWorldTrajectoryEuRoC(const string &filename);
    ...
    // Save TSDF mesh & runtime file
    void SaveRunTime(const string& times_folder, int proccIm, float totalTrack, double MilliSecondsTakenHRC);
    void SaveMesh(const string &meshfile);
