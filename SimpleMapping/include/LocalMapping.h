...
public:
    ...
    LocalMapping(System* pSys, Atlas* pAtlas, const float bMonocular, bool bInertial, const string &_strSeqName=std::string(), bool bDepthConstrain_BA=false);

    // Initialize MVS, TSDF model
    void InitDr(const string &mvsnet_folder, Settings* settings);
    ...
    // mvsnet stuff
    bool mvsnet_flag = false;
    bool dr_timing = false;
    Timer dr_timer;
    vector<KeyFrame*> getCurAtlasKeyframe(){return mpAtlas->GetAllKeyFrames();};
    int curMvsFrameId;
    
    std::string path_save_sparsedepth = "./result/debug/seq_x/sparse/";
...
protected:
    ...
    // pass data to mvsnet
    void DeliverKeyFrameToBackend(KeyFrame *kf, int id, cv::Mat& cur_bgr, int height, int width);
    void SaveRealTimePose(KeyFrame *kf, const string& kf_tuple);
    vector<KeyFrame*> SrcFrameSelection(KeyFrame* kf, vector<KeyFrame*> cov_KeyFrames);
    
    System *mpSystem;

    bool mbMonocular;
    bool mbInertial;
    bool mbDepthConstrain_BA; // Whether to enforce the depth constraint from MVS depth prediction in BA.
    ...
    // mvsnet model, TSDF, backend
    DrMvsnet *mvsnet;
    int dr_width = -1;
    int dr_height = -1;
    int dr_mvsnet_view_num = -1;
    DrFusion *fusion = nullptr;
    MVSBackend *mvs_backend;
    string mstrSeqName;
