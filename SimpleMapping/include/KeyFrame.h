...
public:
  ...
  TrackingDepthMap(int width, int height) {
...
class KeyFrame
{
    ...
    void serialize(Archive& ar, const unsigned int version)
    {
        ...
        // new: image const cast
        serializeMatrix(ar, mImBGR, version); 
    }
...
public:
    ...
    // Get sparse depth(for mvsbackend inputs) & track depth(from mvsbackend)
    cv::Mat GetSparseDepth(int height, int width, const string strSeqName);
    float*  getTrackDepth();
    float getTrackDepth_atpixel(const int u, const int v);
    void SetTrackDepth(TrackingDepthMap* tracker_depth_map_valid);
    // void pushFusionKfImage(float * pdepth, const double timestamp, int count);
    ...
    cv::Mat mImGray;
    // cv::Mat mTrackDepth;
    float* mTrackDepth; // Dense depth predicted from MVS
    bool b_hasTrackDepth = false;
    int mTrackDepth_width;
    int mTrackDepth_height;
...
protected:
    ...
    std::mutex mMutexMap;
    std::mutex mMutexDepth; //new: for dense depth
