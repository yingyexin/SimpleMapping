...
public:
    ...
    Eigen::Vector3d Transform(const Eigen::Vector3d &Xw, int cam_idx=0) const;
...
// The dim of measurement is 1 and in double type.
class EdgeDepthPoseConstrain : public g2o::BaseBinaryEdge<1,double,g2o::VertexSBAPointXYZ,VertexPose>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW

    EdgeDepthPoseConstrain(int cam_idx_=0): cam_idx(cam_idx_){
    }
    ...
    void computeError(){
        ...
        Eigen::Vector3d  Xc(VPose->estimate().Transform(VPoint->estimate(),cam_idx));
        _error(0,0) = obs - Xc(2);
    }
    ...
    Eigen::Matrix<double,1,9> GetJacobian(){
        linearizeOplus();
        Eigen::Matrix<double,1,9> J;
        J.block<1,3>(0,0) = _jacobianOplusXi;
        J.block<1,6>(0,3) = _jacobianOplusXj;
        return J;
    }
...
// The dim of measurement is 1 and in double type.
class EdgeOnlyDepthConstrain : public g2o::BaseUnaryEdge<1,double,g2o::VertexSBAPointXYZ>
        {
        public:
            EIGEN_MAKE_ALIGNED_OPERATOR_NEW

            EdgeOnlyDepthConstrain(ImuCamPose &Pose_, int cam_idx_=0): Pose(Pose_), cam_idx(cam_idx_){
            }
            ...
            void computeError(){
                const g2o::VertexSBAPointXYZ* VPoint = static_cast<const g2o::VertexSBAPointXYZ*>(_vertices[0]);
                const double obs(_measurement);
                Eigen::Vector3d  Xc(Pose.Transform(VPoint->estimate(),cam_idx));
                _error(0,0) = obs - Xc(2);
            }
            ...
            {
                const g2o::VertexSBAPointXYZ* VPoint = static_cast<const g2o::VertexSBAPointXYZ*>(_vertices[0]);
                return Pose.isDepthPositive(VPoint->estimate(),cam_idx);
            }

            Eigen::Matrix<double,1,3> GetJacobian(){
                linearizeOplus();
                Eigen::Matrix<double,1,3> J = _jacobianOplusXi;
                return J;
            }
