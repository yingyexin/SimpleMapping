...
public:
    ...
    // new: Constructor for Monocular cameras.
    Frame(const cv::Mat &imGray, const cv::Mat &imBGR, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());
