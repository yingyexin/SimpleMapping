...
System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
               const bool bUseViewer, const int initFr, const string &strSequence, const string &strModelFile):
    mSensor(sensor), mpViewer(static_cast<Viewer*>(NULL)), mbReset(false), mbResetActiveMap(false),
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    // Output welcome message
    cout << endl <<
    "SimpleMapping Copyright (C) 2023 Yingye Xin, Xingxing Zuo, Dongyue Lu and Stefan Leutenegger, Technical University of Munich." << endl <<
    ...
    node = fsSettings["depthConstrain_BA"];
    bool bDepthConstrain_BA = false;
    if(!node.empty())
    {
        bDepthConstrain_BA = static_cast<int>(fsSettings["depthConstrain_BA"]) != 0;
    }
    cout<<"Enable DepthConstrain_BA: "<<bDepthConstrain_BA<<endl;
    ...
    mpLocalMapper = new LocalMapping(this, mpAtlas, mSensor==MONOCULAR || mSensor==IMU_MONOCULAR,
                                     mSensor==IMU_MONOCULAR || mSensor==IMU_STEREO || mSensor==IMU_RGBD, strSequence, bDepthConstrain_BA);
    
    //Initialize MVSNet, TSDF-Fusion and MvsBackend
    mpLocalMapper->InitDr(strModelFile, settings_);
    ...
    //Initialize and launch the Viewer thread for the whole program and/ MvsBackend
    if(bUseViewer)
    {
        ...
        // Visualize MVSNet predicted depth
        if (settings_->mvsDepth()) mpLocalMapper->SetFrameDrawer(mpFrameDrawer);
    }
...
Sophus::SE3f System::TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    // undistort the resized img
    cv::Mat imdistort = imToFeed.clone(); 
    cv::Mat imUndistort;
    if (settings_ && settings_->needToUndistort()){
        cv::Mat DistCoef = settings_->camera1DistortionCoef();
        cv::Mat K = (cv::Mat_<float>(3, 3)
            << settings_->camera1()->getParameter(0), 0.f, settings_->camera1()->getParameter(2), 
            0.f, settings_->camera1()->getParameter(1), settings_->camera1()->getParameter(3), 0.f, 0.f, 1.f);
        cv::undistort(imdistort, imUndistort, K, DistCoef);
    }
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageMonocular(imToFeed, imUndistort, timestamp,filename);
...
// Save trajectory wrt. world frame instead of first keyframe
void System::SaveWorldTrajectoryEuRoC(const string &filename)
{

    cout << endl << "Saving world trajectory to " << filename << " ..." << endl;
    ...
    // keep the new world be the same world coordinate
    Sophus::SE3f Twb; 
        ...
        lend=mpTracker->mlRelativeFramePoses.end();lit!=lend;lit++, lRit++, lT++, lbL++)
    {
        ...
        KeyFrame* pKF = *lRit; // reference frame 
        //cout << "KF: " << pKF->mnId << endl;
        ...
        Trw = Trw * pKF->GetPose()*Twb; // Tcp*Tpw*Twb0=Tcb0 where b0 is the new world reference, reference to new world(first kf)

        // cout << "4" << endl;
        if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO || mSensor==IMU_RGBD)
        {
            Sophus::SE3f Twb = (pKF->mImuCalib.mTbc * (*lit) * Trw).inverse();//frame to reference to new world
            Eigen::Quaternionf q = Twb.unit_quaternion();
            Eigen::Vector3f twb = Twb.translation();
            f << setprecision(6) << 1e9*(*lT) << " " <<  setprecision(9) << twb(0) << " " << twb(1) << " " << twb(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
        }
    ...
    f.close();
    cout << endl << "End of saving world trajectory to " << filename << " ..." << endl;
}
...
// Save runtime after all frames
void System::SaveRunTime(const string& times_folder, int proccIm, float totalTrack, double MilliSecondsTakenHRC){

    if(mpLocalMapper->dr_timing){
        ofstream f;
        f.open((times_folder + "dr_times.txt").c_str());
        f << fixed;
        f << setprecision(9) << proccIm << "Frames, " << 1000 * proccIm / MilliSecondsTakenHRC  << "fps, " 
              << totalTrack / proccIm << "ms Tracking perframe, " << MilliSecondsTakenHRC / proccIm  
              << "ms per frame " << MilliSecondsTakenHRC / 1000.0 << "s total time. " << endl;
        mpLocalMapper->dr_timer.write_to_file(times_folder + "dr_times.txt");
    }
}

// Save mesh after all frames
void System::SaveMesh(const string& mesh_file){
        
    float mesh_lower_corner[3] = {-8, -8, -4};
    float mesh_upper_corner[3] = {8, 8, 4};

    std::string filename_mesh = mesh_file + "mesh.obj.incomplete";
    std::string filename_mesh_c = mesh_file + "mesh.obj";
    cout << endl << "Saving mesh to " << filename_mesh_c << endl;

    mpLocalMapper->getFusion()->SaveMeshToFile(filename_mesh, mesh_lower_corner, mesh_upper_corner);
    mpLocalMapper->getFusion()->Synchronize();
    std::rename(filename_mesh.c_str(), filename_mesh_c.c_str());
