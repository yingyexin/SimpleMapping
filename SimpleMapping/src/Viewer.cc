...
Viewer::Viewer(System* pSystem, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Tracking *pTracking, const string &strSettingPath, Settings* settings):
    ...
    mbFinishRequested(false), mbFinished(true), mbStopped(true), mbStopRequested(false), mbMvsDepth(false)
{
    if(settings){
...
void Viewer::newParameterLoader(Settings *settings) {
    ...
    mbMvsDepth = settings->mvsDepth(); // For mvsbackend visualization
}
...
void Viewer::Run()
{
    ...
    pangolin::CreateWindowAndBind("SimpleMapping: Map Viewer",1024,768);
    ...
    Ow.SetIdentity();
    cv::namedWindow("SimpleMapping: Current Frame");
    // mvs predicted depth visualization
    if (mbMvsDepth){
        cv::namedWindow("SimpleMapping: MVS Key Frame"); //create new window, refer by name
        cv::namedWindow("SimpleMapping: MVS Predicted Depth");
    }
    ...
    while(1)
    {
        ...
        if (mpFrameDrawer->dr_mesh_num > 0){
            pangolin::glDrawColoredVertices(
                mpFrameDrawer->dr_mesh_num, mpFrameDrawer->getDrMeshVert(), mpFrameDrawer->getDrMeshCols(),
                GL_TRIANGLES, 3, 3);
            
            mpFrameDrawer->bMeshChanged = false;
        }
        ...
        cv::imshow("SimpleMapping: Current Frame",toShow);
        cv::waitKey(mT);

        if (mbMvsDepth && mpFrameDrawer->bFrameReady){
            cv::Mat imKF = mpFrameDrawer->DrawMvsFrame();
            cv::imshow("SimpleMapping: MVS Key Frame", imKF);

            cv::Mat imDepth = mpFrameDrawer->DrawMvsDepth();
            cv::imshow("SimpleMapping: MVS Predicted Depth", imDepth);
            
            cv::waitKey(mT);
            mpFrameDrawer->bFrameReady = false;
        }
