...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges)
{
    ...
    //printf("LocalBundleAdjustment num_OptKF or lLocalKeyFrames.size=%d\n", num_OptKF);
...
void Optimizer::LocalInertialBA(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges, bool bLarge, bool bRecInit, bool bDepthConstrain_BA)
{
    Map* pCurrentMap = pKF->GetMap();
    ...
    vector<EdgeDepthPoseConstrain*> vpEdgesDepthPose;
    vpEdgesDepthPose.reserve(nExpectedSize);

    vector<EdgeOnlyDepthConstrain*> vpEdgesOnlyDepth;
    vpEdgesOnlyDepth.reserve(nExpectedSize);
    ...
    const float thHuberSingleMea = sqrt(3.841);
    const float chi2SingleMea= 3.841;
    const float thHuberMono = sqrt(5.991);
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
            {
                ...
                if(leftIndex != -1 && pKFi->mvuRight[leftIndex]<0)
                {
                    ...
                    /// MVS Depth constraints
                    // TODO: apply the depth constraints for stereo camera ...
                    if(bDepthConstrain_BA && pKFi->b_hasTrackDepth){
                        const double mvs_depth_value = pKFi->getTrackDepth_atpixel(int(kpUn.pt.x), int(kpUn.pt.y));
                        if( mvs_depth_value > 0.2){
                            EdgeDepthPoseConstrain* e_depthpose = new EdgeDepthPoseConstrain(0);
                            e_depthpose->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(id)));
                            e_depthpose->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKFi->mnId)));

//                            ImuCamPose pKFi_Pose = ImuCamPose(pKFi);
//                            EdgeOnlyDepthConstrain* e_depthpose = new EdgeOnlyDepthConstrain(pKFi_Pose, 0);
//                            e_depthpose->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(id)));

                            e_depthpose->setMeasurement(mvs_depth_value);
                            e_depthpose->computeError(); // Never forget this if you wanna get error(), otherwise it can be ignored.
//                            cout<<"DepthConstrain DepthMea, Error: "<< mvs_depth_value <<" "<<abs(e_depthpose->error()(0,0))<<endl;
                            if(abs(e_depthpose->error()(0,0)) < 1.6){
                                Eigen::Matrix<double,1,1> info_e(0.5*invSigma2);//0.025*
                                e_depthpose->setInformation(info_e);
                                g2o::RobustKernelHuber* rk_depthpose = new g2o::RobustKernelHuber;
                                e_depthpose->setRobustKernel(rk_depthpose);
                                rk_depthpose->setDelta(0.4*thHuberMono);
                                vpEdgesDepthPose.push_back(e_depthpose);
//                                vpEdgesOnlyDepth.push_back(e_depthpose);
                                optimizer.addEdge(e_depthpose);
                                // cout<<"For debug, insert EdgeDepthPoseConstrain for KF "<< pKFi->mnId <<", at pixel: "<< int(kpUn.pt.x) <<" "<< int(kpUn.pt.y) << " , with depth value: "<< mvs_depth_value <<endl;
                            }else{
                                delete e_depthpose;
                            }
                        }
    ...
    // cout<<"In LocalInertialBA, the number of vpEdgesDepthPose, and vpEdgesOnlyDepth: "<<vpEdgesDepthPose.size()<<" "<<vpEdgesOnlyDepth.size()<<endl;
    ...
    N=vpOptimizableKFs.size();
    //printf("LocalInertialBA N vpOptimizableKFs.size = %d \n", N);
    for(int i=0; i<N; i++)
    {
        KeyFrame* pKFi = vpOptimizableKFs[i];
