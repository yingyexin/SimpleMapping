...
Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat &im, const cv::Mat &imBGR, const double &timestamp, string filename)
{
    mImGray = im;
    mImBGR = imBGR;// undistort rgb channels = 3
    if(mImGray.channels()==3)
    {
        if(mbRGB){
            cvtColor(mImGray,mImGray,cv::COLOR_RGB2GRAY);
    ...
    if (mSensor == System::MONOCULAR)
    {
        if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET ||(lastID - initID) < mMaxFrames)
            // Pass mImBGR to frame initialization
            mCurrentFrame = Frame(mImGray, mImBGR, timestamp,mpIniORBextractor,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth);
        else
            mCurrentFrame = Frame(mImGray, mImBGR, timestamp,mpORBextractorLeft,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth);
    }
    else if(mSensor == System::IMU_MONOCULAR)
    {
        ...
        {
            // Pass mImBGR to frame initialization
            mCurrentFrame = Frame(mImGray, mImBGR, timestamp,mpIniORBextractor,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth,&mLastFrame,*mpImuCalib);
        }
        else
            mCurrentFrame = Frame(mImGray, mImBGR, timestamp,mpORBextractorLeft,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth,&mLastFrame,*mpImuCalib);
    }
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                {
                    // printf("TRACK: Track with respect to the reference KF ");
                    Verbose::PrintMess("TRACK: Track with respect to the reference KF ", Verbose::VERBOSITY_DEBUG);
                    bOK = TrackReferenceKeyFrame();
                }
                else
                {
                    // printf("TRACK: Track with motion model ");
...
bool Tracking::TrackLocalMap()
{
    ...
    if (mSensor == System::IMU_MONOCULAR)
    {
        ...
        {   
            cout << mCurrentFrame.mnId << " " << mnMatchesInliers << " " << (mpAtlas->isImuInitialized())<< endl;
            return false;
        }
        else
            return true;
...
void Tracking::CreateNewKeyFrame()
{
    ...
    {
        printf("createNewKeyFrame using mvDepth.");
