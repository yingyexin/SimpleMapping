...
//new: add mvs keyframe
void Map::AddMvsKeyFrame(KeyFrame *pKF)
{
    unique_lock<mutex> lock(mMutexMap);
    if(mspMvsKeyFrames.empty())
        cout << "First MVSnet KF:" << pKF->mnId << endl;
    mspMvsKeyFrames.insert(pKF);
}
...
//new: get mvs keyframe
vector<KeyFrame*> Map::GetMvsKeyFrames()
{
    unique_lock<mutex> lock(mMutexMap);
    return vector<KeyFrame*>(mspMvsKeyFrames.begin(),mspMvsKeyFrames.end());
}
