...
namespace ORB_SLAM3 {
    ...
    bNeedToUndistort_(false), bNeedToRectify_(false), bNeedToResize1_(false), bNeedToResize2_(false) {
        ...
        // Load MVSNet parameter
        readMvsnetParameters(fSettings);
        cout << "\t-Loaded MVSNet parameters" << endl;
    ...
    void Settings::readImageInfo(cv::FileStorage &fSettings) {
        ...
        int newHeight = readParameter<int>(fSettings,"Camera.newHeight",foundH,false);
        if(foundH){
        ...
        int newWidth = readParameter<int>(fSettings,"Camera.newWidth",foundW,false);
        if(foundW){
            bNeedToResize1_ = true;
    ...
    void Settings::readViewer(cv::FileStorage &fSettings) {
        ...
        // Viewer for mvsdepth
        bMvsDepth_ = (bool) readParameter<int>(fSettings,"Viewer.MvsDepth",found,false);
    }
    ...
    void Settings::readMvsnetParameters(cv::FileStorage& fSettings){
        bool found;

        bMvsFlag_ = (bool) readParameter<int>(fSettings,"mvsnet_flag",found,false);
        bSettingTsdfFusion_ = (bool) readParameter<int>(fSettings,"setting_tsdf_fusion",found,false);
        nMvsnetView_ = readParameter<int>(fSettings,"dr_mvsnet_view_num",found,false);
        nMvsnetRenderStream_ = readParameter<int>(fSettings,"dr_mvsnet_render_stream",found,false);
        bRuntimeEval_ = (bool) readParameter<int>(fSettings,"dr_timing",found,false);
    }
