...
Eigen::Vector3d ImuCamPose::Transform(const Eigen::Vector3d &Xw, int cam_idx) const
{
    Eigen::Vector3d Xc = Rcw[cam_idx] * Xw + tcw[cam_idx];
    return Xc;
}
...
void EdgeDepthPoseConstrain::linearizeOplus()
{
    const VertexPose* VPose = static_cast<const VertexPose*>(_vertices[1]);
    const g2o::VertexSBAPointXYZ* VPoint = static_cast<const g2o::VertexSBAPointXYZ*>(_vertices[0]);
...
//    const Eigen::Matrix<double,2,3> proj_jac = VPose->estimate().pCamera[cam_idx]->projectJac(Xc);
    ...
    _jacobianOplusXj = Rcb.row(2) * SE3deriv;
}

void EdgeOnlyDepthConstrain::linearizeOplus()
{
    const g2o::VertexSBAPointXYZ* VPoint = static_cast<const g2o::VertexSBAPointXYZ*>(_vertices[0]);

    const Eigen::Matrix3d &Rcw = Pose.Rcw[cam_idx];
//    const Eigen::Vector3d &tcw = Pose.tcw[cam_idx];
//    const Eigen::Vector3d Xc = Rcw*VPoint->estimate() + tcw;
//    const Eigen::Vector3d Xb = Pose.Rbc[cam_idx]*Xc+Pose.tbc[cam_idx];
//    const Eigen::Matrix3d &Rcb = Pose.Rcb[cam_idx];
