...
// new: frame definition for bgr
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imBGR, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, Frame* pPrevF, const IMU::Calib &ImuCalib)
    :mpcpi(NULL),mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
     ...
     mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    // new: store image(gray & bgr) for mvsnet
    mImBGR = imBGR;
    mImGray = imGray;

    // cv::Mat distwkp;
    // cv::drawKeypoints(mImGray, mvKeys, distwkp);
    // cv::imshow("mImGray with keypoints", distwkp);
    // cv::waitKey(1);
    // cv::Mat undistwkp;
    // cv::drawKeypoints(mImBGR, mvKeysUn, undistwkp);
    // cv::imshow("mImBGR with Unkeypoints", undistwkp);
    // cv::waitKey(1);
