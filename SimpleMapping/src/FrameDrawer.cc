...
FrameDrawer::FrameDrawer(Atlas* pAtlas, Settings* settings):both(false),mpAtlas(pAtlas)
{
    ...
    mHeight = imSize.height;
    mWidth = imSize.width;
    mIm = cv::Mat(mHeight,mWidth,CV_8UC3, cv::Scalar(0,0,0));
    mImRight = cv::Mat(mHeight,mWidth,CV_8UC3, cv::Scalar(0,0,0));

    // for mvs visualization
    mbMvsDepth = settings->mvsDepth();
    if (mbMvsDepth){
        mImBGR = cv::Mat(mHeight,mWidth,CV_8UC3, cv::Scalar(0,0,0));
        mInternalDepth = cv::Mat(mHeight,mWidth,CV_8UC3, cv::Scalar(0,0,0));

        internalDrKfImage = new MinimalImageB3(mWidth, mHeight);
        internalDrKfDepth = new MinimalImageB3(mWidth, mHeight);
...
// MvsBackend update keyframe
void FrameDrawer::pushDrKfImage(unsigned char * bgr, const double timestamp)
{
  ...
  mImBGR = internalKfImage;
}

void FrameDrawer::pushDrKfDepth(float * image, float depth_min, float depth_max, const double timestamp)
{
  for (int _h = 0; _h < height; _h++) {
    for (int _w = 0; _w < width; _w++) {
      const int i = _h*width + _w;
  ...
  cv::Mat denseDepth = cv::Mat(height, width, CV_32F, image);
  cv::Mat internalDepth = cv::Mat(height, width, CV_8UC3, internalDrKfDepth->data);
  mInternalDepth = internalDepth;
  bFrameReady = true;
}

// visualize the MvsBackend mesh
void FrameDrawer::pushDrMesh(size_t num, float const* vert, float const* cols){
